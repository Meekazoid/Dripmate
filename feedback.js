...
export function initFeedbackSliderInteractions() {
    ...
    // Debounced slightly so touchmove can detect vertical scroll before we commit
    ...
    document.addEventListener('input', (event) => {
        ...
        // If touch is active but direction not yet decided, defer the update
        if (state && !state.decided) {
            clearTimeout(inputDebounceTimer);
            inputDebounceTimer = setTimeout(() => {
                const currentState = feedbackTouchState.get(slider);
    ...
    document.addEventListener('touchmove', (event) => {
        ...
        if (deltaY > 12 && deltaY > deltaX) {
            ...
            updateSliderVisual(slider);
            // Re-trigger suggestion so stale Strategy Box gets cleared
            const [index, category] = String(slider.dataset.feedbackSlider || '').split('-');
            if (index !== undefined && category) {
                updateFeedbackSlider(Number(index), category, state.startValue);
            }
        } else if (deltaX > 8) {
    ...
    document.addEventListener('touchend', (event) => {
        ...
        if (state?.cancelled) {
            ...
            slider.value = state.startValue;
            updateSliderVisual(slider);
            // Ensure suggestion matches the restored value
            const [index, category] = String(slider.dataset.feedbackSlider || '').split('-');
            if (index !== undefined && category) {
                updateFeedbackSlider(Number(index), category, state.startValue);
            }
